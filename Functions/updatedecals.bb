Function updatedecals%()
    ...
    For local0 = Each decals
        If (0.0 <> local0\Field1) Then
            ...
            If (local1 = $00) Then
                If (0.0 >= local0\Field7) Then
                    ...
                    local0\Field7 = (Float rand($1E, $64))
                    scalesprite(local4\Field0, local4\Field2, local4\Field2)
                    playsound2(decaysfx(rand($01, $03)), camera, local4\Field0, 10.0, rnd(0.1, 0.5), $01)
                Else
                    local0\Field7 = (local0\Field7 - fpsfactor)
                EndIf
            EndIf
        ...
        If (0.0 < local0\Field8) Then
            local0\Field8 = max((local0\Field8 - fpsfactor), 5.0)
        EndIf
        If ((((0.0 >= local0\Field2) Or (0.0 >= local0\Field4)) Or ((0.0 < local0\Field8) And (10.0 > local0\Field8))) <> 0) Then
